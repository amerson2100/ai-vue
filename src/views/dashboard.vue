        ...
        <el-card v-if="aiData.consultationStats">
          <div class="card-content">
            ...
            <div class="info">
              ...
              <p class="value">{{ aiData.consultationStats.totalSessions }}</p>
              <p class="subtitle-title">
                今日新增：{{ aiData.consultationStats.todayNewSessions || 0 }}
              </p>
            </div>
          </div>
        </el-card>
...
onMounted(() => {
  getAnalyticsOverview().then((res) => {
    ...
    console.log("Consultation stats:", res.consultationStats);
    aiData.value = res;
    initCharts();
  });
});
